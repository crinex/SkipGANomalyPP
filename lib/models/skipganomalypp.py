...
class Skipganomalypp(BaseModel):
    """GANomaly Class
    """
    @property
    def name(self): return 'skipganomalypp'

    def __init__(self, opt, data=None):
        super(Skipganomalypp, self).__init__(opt, data)
        ##
        ...
        if self.opt.resume != '':
            ...
            self.opt.iter = torch.load(os.path.join(self.opt.resume, 'netG_45.pth'))['epoch']
            self.netg.load_state_dict(torch.load(os.path.join(self.opt.resume, 'netG_45.pth'))['state_dict'])
            self.netd.load_state_dict(torch.load(os.path.join(self.opt.resume, 'netD_45.pth'))['state_dict'])
            print("\tDone.\n")
    ...
    def test(self, plot_hist=True):
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(self.data.valid, 0):
                self.total_steps = self.total_steps + self.opt.batchsize
                epoch_iter = epoch_iter + self.opt.batchsize
                time_i = time.time()
                ...
                self.gt_labels[i*self.opt.batchsize: i*self.opt.batchsize + error.size(0)] = self.gt.reshape(error.size(0)) # 정답 라벨?
            ...
            ap = evaluate(self.gt_labels, self.an_scores, 'auprc')
            #performance = OrderedDict([('Avg Run Time (ms/batch)', self.times), ('AUPRC', ap)])
            #print(performance)

            f1 = evaluate(self.gt_labels, self.an_scores, 'f1_score')
            #performance = OrderedDict([('Avg Run Time (ms/batch)', self.times), ('f1_score', f1)])
            #print(performance)
            
            acc = evaluate(self.gt_labels, self.an_scores, 'accuracy')
            # performance = OrderedDict([('Avg Run Time (ms/batch)', self.times), ('Accuracy', acc)])
            # print(performance)

            sens = evaluate(self.gt_labels, self.an_scores, 'sens')
            # performance = OrderedDict([('Avg Run Time (ms/batch)', self.times), ('Sensitivity', sens)])
            # print(performance)

            spec = evaluate(self.gt_labels, self.an_scores, 'spec')
            # performance = OrderedDict([('Avg Run Time (ms/batch)', self.times), ('Specificity', spec)])
            # print(performance)
            performance = OrderedDict([('AUC', auc), ('AUPRC', ap), ('f1_score', f1), ('Accuracy', acc), ('Sensitivity', sens), ('Specificity', spec)])
            ...
            if plot_hist:
                ...
                # plt.savefig(os.path.join(self.opt.outf, self.opt.name, 'test', 'histogram.png'))
